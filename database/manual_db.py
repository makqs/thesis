#!/usr/bin/env python3

import psycopg2, re

# connect to progression checker db
conn = psycopg2.connect(
    host="127.0.0.1",
    database="pc",
    user="maxowen")
cursor = conn.cursor()

cursor.execute(open("db_schema3.sql", "r").read())

cursor.execute(f"""INSERT INTO users(zid, name, is_staff)
                   VALUES ('z5263663', 'Max Owen', false)""")

cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('1', 'COMP1511', 'Programming Fundamentals', 2019, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('2', 'COMP1521', 'Computer Systems Fundamentals', 2019, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('3', 'COMP1531', 'Software Engineering Fundamentals', 2019, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('4', 'ENGG1000', 'Introduction to Engineering Design and Innovation', 2019, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('5', 'MATH1081', 'Discrete Mathematics', 2019, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('6', 'MATH1131', 'Mathematics 1A', 2019, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('7', 'MATH1141', 'Higher Mathematics 1A', 2019, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('8', 'MATH1231', 'Mathematics 1B', 2019, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('9', 'MATH1241', 'Higher Mathematics 1B', 2019, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('10', 'COMP2041', 'Software Construction: Techniques and Tools', 2020, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('11', 'COMP2511', 'Object-Oriented Design & Programming', 2020, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('12', 'COMP2521', 'Data Structures and Algorithms', 2019, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('13', 'DESN2000', 'Engineering Design and Professional Practice', 2020, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('14', 'MATH2400', 'Finite Mathematics', 2020, 3)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('15', 'MATH2859', 'Probability, Statistics and Information', 2020, 3)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('16', 'SENG2011', 'Workshop on Reasoning about Programs', 2020, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('17', 'SENG2021', 'Requirements and Design Workshop', 2020, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('18', 'COMP3141', 'Software System Design and Implementation', 2021, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('19', 'COMP3311', 'Database Systems', 2020, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('20', 'COMP3331', 'Computer Networks and Applications', 2020, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('21', 'SENG3011', 'Software Engineering Workshop 3', 2021, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('22', 'SENG4920', 'Ethics and Managements', 2022, 6)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('23', 'COMP4951', 'Research Thesis A', 2022, 4)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('24', 'COMP4952', 'Research Thesis B', 2022, 4)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('25', 'COMP4953', 'Research Thesis C', 2022, 4)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('26', 'ENGG2600', 'Engineering Vertically Integrated Project', 2022, 2)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('27', 'ENGG3600', 'Engineering Vertically Integrated Project', 2022, 2)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('28', 'ENGG4600', 'Engineering Vertically Integrated Project', 2022, 2)""")
cursor.execute(f"""INSERT INTO courses(course_id, code, title, year, uoc)
                   VALUES ('29', 'ENGG3060', 'Maker Games', 2022, 3)""")

cursor.execute(f"""INSERT INTO programs(program_id, year, code, title, total_uoc)
                   VALUES ('1', 2021, '3707', 'Engineering (Honours)', 192)""")

cursor.execute(f"""INSERT INTO streams(stream_id, year, code, title, total_uoc)
                   VALUES ('1', 2021, 'SENGAH', 'Software Engineering', 168)""")

cursor.execute(f"""INSERT INTO program_rules(program_rule_id, program_id, name, type, min_uoc, max_uoc, definition)
                   VALUES ('1', '1', 'Stream', 'ST', 168, 168, 'AEROAH,BINFAH,CEICAH,CEICDH,COMPBH,CVENAH,CVENBH,ELECAH,MECHAH,SENGAH')""")
cursor.execute(f"""INSERT INTO program_rules(program_rule_id, program_id, name, type, min_uoc, max_uoc, definition)
                   VALUES ('2', '1', 'General Education', 'GE', 12, 12, '')""")
cursor.execute(f"""INSERT INTO program_rules(program_rule_id, program_id, name, type, min_uoc, max_uoc, definition)
                   VALUES ('3', '1', 'Discipline Electives', 'DE', 12, 12, '')""")

# cursor.execute(f"""INSERT INTO stream_rules(stream_rule_id, stream_id, name, type, min_uoc, max_uoc, definition)
#                    VALUES ('1', '1', 'Level 1 Core Courses', 'CC', 42, 42, 'COMP1511,COMP1521,COMP1531,ENGG1000,MATH1081,{{MATH1131;MATH1141}},{{MATH1231;MATH1241}}')""")
cursor.execute(f"""INSERT INTO stream_rules(stream_rule_id, stream_id, name, type, min_uoc, max_uoc, definition)
                   VALUES ('1', '1', 'Level 1 Core Courses', 'CC', 42, 42, 'COMP1511,COMP1521,COMP1531,ENGG1000,MATH1081,MATH1131,MATH1141,MATH1231,MATH1241')""")
cursor.execute(f"""INSERT INTO stream_rules(stream_rule_id, stream_id, name, type, min_uoc, max_uoc, definition)
                   VALUES ('2', '1', 'Level 2 Core Courses', 'CC', 42, 42, 'COMP2041,COMP2511,COMP2521,DESN2000,MATH2400,MATH2859,SENG2011,SENG2021')""")
cursor.execute(f"""INSERT INTO stream_rules(stream_rule_id, stream_id, name, type, min_uoc, max_uoc, definition)
                   VALUES ('3', '1', 'Level 3 Core Courses', 'CC', 24, 24, 'COMP3141,COMP3311,COMP3331,SENG3011')""")
cursor.execute(f"""INSERT INTO stream_rules(stream_rule_id, stream_id, name, type, min_uoc, max_uoc, definition)
                   VALUES ('4', '1', 'Level 4 Core Courses', 'CC', 18, 18, 'SENG4920,COMP4951,COMP4952,COMP4953')""")
cursor.execute(f"""INSERT INTO stream_rules(stream_rule_id, stream_id, name, type, min_uoc, max_uoc, definition)
                   VALUES ('5', '1', 'Discipline Electives', 'DE', 36, 36, 'ENGG2600,ENGG3600,ENGG4600,COMP3XXX,COMP4XXX,COMP6XXX,COMP9XXX,INFS3XXX,INFS4XXX,MATH3XXX,MATH4XXX,MATH6XXX,ELEC3XXX,ELEC4XXX,TELE3XXX,TELE4XXX')""")
cursor.execute(f"""INSERT INTO stream_rules(stream_rule_id, stream_id, name, type, min_uoc, max_uoc, definition)
                   VALUES ('6', '1', 'Free Electives', 'FE', 6, NULL, NULL)""")

cursor.execute(f"""INSERT INTO program_enrolments(zid, program_id)
                   VALUES ('z5263663', '1')""")

cursor.execute(f"""INSERT INTO stream_enrolments(zid, stream_id)
                   VALUES ('z5263663', '1')""")

cursor.execute(f"""INSERT INTO course_enrolments(zid, course_id, mark, grade)
                   VALUES ('z5263663', '1', 84, 'DN')""")
cursor.execute(f"""INSERT INTO course_enrolments(zid, course_id, mark, grade)
                   VALUES ('z5263663', '2', 77, 'DN')""")
cursor.execute(f"""INSERT INTO course_enrolments(zid, course_id, mark, grade)
                   VALUES ('z5263663', '3', 85, 'FL')""")
cursor.execute(f"""INSERT INTO course_enrolments(zid, course_id, mark, grade)
                   VALUES ('z5263663', '14', 78, 'DN')""")

cursor.close()
conn.commit()
conn.close()
